...
def survived_bar_chart(dataset, pass_class, cpalette=palette[1:3]):
    ...
    p = figure(tools=[hover_tool], plot_height=400,
               title='Did/Did not Survive for Current Class')
...
def class_titles_bar_chart(dataset, pass_class, cpalette=palette):
    ...
    p = figure(tools=[hover_tool], plot_height=300,
               title='Titles for Current Class')
...
def age_hist(dataset, pass_class, color=palette[1]):
    hist, edges = np.histogram(dataset[dataset['Pclass'] == int(
        pass_class)]['Age'].fillna(df['Age'].mean()), bins=25)
    ...
    hover_tool = HoverTool(
        tooltips=[('From', '@edges_left'),
                  ('Thru', '@edges_right'), ('Count', '@hist')],
        mode='vline'
    )
